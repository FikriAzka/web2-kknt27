...
import { ChefHat, Heart, Users, Clock, Utensils } from "lucide-react";
...
const RecipeCard = ({ recipe }: RecipeCardProps) => (
  <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 w-full">
    <div className="bg-gradient-to-r from-orange-400 to-red-400 p-6 text-white">
      <h3 className="text-2xl lg:text-3xl font-bold flex items-center justify-center">
              ...
              <ol className="space-y-3">
                {recipe.instructions.map((step, index) => (
                  <li key={index} className="text-gray-700">
                    <span className="bg-orange-500 text-white rounded-full w-7 h-7 inline-flex items-center justify-center text-base font-bold mr-3 flex-shrink-0">
                      {index + 1}
                    </span>
                    {step}
                  </li>
...
export default function OlahanLelePage() {
  // activeTab tetap spesifik
  const [activeTab, setActiveTab] = useState<RecipeKey>("bakso");

  // handler terpisah untuk klik tab
  const handleTabClick = (id: string) => {
    if (id in recipes) {
      setActiveTab(id as RecipeKey);
    }
  };
